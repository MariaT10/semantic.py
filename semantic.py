...
# Interesting Note:
# monkey and banana have a higher similarity than monkey and apple.
# but cat and apple don't have a high similarity

print("\nExercise 3: \n")
tokens = nlp('milk cookie fridge breakfast ')
for token1 in tokens:
    for token2 in tokens:
        print(token1.text, token2.text, token1.similarity(token2))

# Interesting Note:
# the highest similarity is between breakfast and fridge
...
# Interesting Note:
# the highest similarity is between "Why is my cat on the car" and "Hello, there is my car"
# because it assumes that if you can see the car, you can also see that the cat is on the car.
...
# We will now compare the similarity of the complaints.
...
# Interesting Note:
# compared to 'en_core_web_md'
# the 'en_core_web_sm' model also prints a User Warning Message:

# [W007] The model you're using has no word vectors loaded, so the result of the Doc.similarity method will be based on the tagger, 
...
# which means that the similarities displayed might not be as accurate due to the used model